...
- `core/geometry.py` — shape volume formulas (`vol_cuboid`, `vol_cylinder`, `vol_truncated_cone`, `vol_truncated_rect_pyramid`) plus enclosure-geometry aggregation (`segment_volume`, `enclosure_volume_and_length`, `equivalent_diameter`, `ld_ratio`) used to turn a stack of shape segments into a total V/L/D.
